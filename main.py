...
class AIContentBot:
    ...
    def generate_and_check_content(self, content, user_id, media_url=None):
        user_interests = self.user_interests.get_user_interests(user_id)
        
        generated_content = self.generator.generate_content(content, user_interests)
        if media_url:
            ...
            if bias_detected:
                return False, f"Content cannot be posted due to detected bias: {bias_result}"
